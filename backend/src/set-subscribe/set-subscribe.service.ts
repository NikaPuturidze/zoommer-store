...
@Injectable()
export class SetSubscribeService {
  async setSubscribe(lang: string, accessToken: string, subscribe: string): Promise<unknown> {
    ...
    const path = `/v1/Users/subscribe-unsubscribe?subscribe=${subscribe}`
    const requestBody = JSON.stringify({ subscribe })
    ...
    return new Promise((resolve, reject) => {
      ...
      request.on('error', () => {
        reject(new BadRequestException('Failed to fetch setSubscribe'))
      })
